...
public class Optical_Network_networkCost implements IReport
{
	...
	public String executeReport(NetPlan netPlan, Map<String, String> reportParameters, Map<String, String> net2planParameters)
	{
		...
		double opticalChannels[] = new double[(int) linkNumber];
		...
		if (E > 0)
		{
			...
			for (long linkId : linkIds)
			{
				...
				for(int i=id ; i<linkNumber ; i++)
				{
					for(int j=0 ; j<idLinks.length ; j++) 
					{
						if(i==idLinks[j]) break;
						if(j==idLinks.length-1)
						{
							j=0;
							i++;
						}
					...
					Link l1=netPlan.getLinkFromId(i);
					String originNode1 = l1.getOriginNode().getName();
					String destinationNode1 = l.getDestinationNode().getName();
